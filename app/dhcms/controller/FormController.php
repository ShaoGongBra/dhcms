...
class FormController extends SiteController {
    ...
    public function index(){
        ...
        //位置导航
        $crumb = array(array('name'=>$formInfo['name'],'url'=>url('dhcms/Form/index',$pageMaps)));
        //MEDIA信息
    ...
    public function info(){
        ...
        //格式化表单内容为基本数据
        foreach ($fieldList as $v) {
            $info[$v['field']] = target('dhcms/FieldData')->revertField($info[$v['field']],$v['type'],$v['config']);
        }
        //位置导航
        $crumb = array(
            array('name'=>$formInfo['name'],'url'=>url('dhcms/Form/index',array('name'=>$name))),
            array('name'=>'详情','url'=>url('dhcms/Form/info',array('name'=>$name,'id'=>$id))),
            );
        //MEDIA信息
