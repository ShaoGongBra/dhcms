...
class AdminFragmentController extends AdminController {
    ...
    public function index(){
        ...
        $list = target('Fragment')->loadList();
        $typeField = target('Fragment')->typeField();
        foreach ($list as $key => $value) {
           $list[$key]['type'] = $typeField[$value['type']]['name'];
        }
    ...
    public function add(){
        if(!IS_POST){
            ...
            $this->assign('breadCrumb',$breadCrumb);
            $this->assign('name','添加');
            $this->assign('typeField', target('Fragment')->typeField());
    ...
    public function getField()
    {
        ...
        $fragmentId = request('post.fragment_id');

        $fieldModel = target('Fragment');
        $info = $fieldModel->getInfo($fragmentId);
        if (empty($type)) {
           $type = $info['type'];
        }

        $html = $fieldModel->htmlFieldFull($type,$info['content']);    
        $this->show($html);
    }
