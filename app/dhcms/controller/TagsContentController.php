    ...
    public function index(){
        ...
        if(!empty($list)){
            ...
            foreach ($list as $key => $value) {
                $data[$key]=$value;
                $data[$key]['curl']=target('dhcms/Category')->getUrl($value);
                $data[$key]['aurl']=target('dhcms/Content')->getUrl($value);

            }
        }
        //位置导航
        $crumb = array(
            array('name'=>'标签列表','url'=>url('dhcms/Tags/index')),
            array('name'=>$tagInfo['name'],'url'=>url('dhcms/TagsContent/index',array('name'=>$tagInfo['name']))),
            );
        //URL参数
        $pageMaps = array();
        $pageMaps['name'] = $tag;
		...
		$this->assign('pageInfo',$this->getPage($pageMaps));
