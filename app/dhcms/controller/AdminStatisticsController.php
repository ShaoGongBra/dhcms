...
class AdminStatisticsController extends AdminController {
    ...
    public function SpiderInfo(){
            ...
            //查询数据
            $list = target('dhcms/TotalSpiderInfo')->page(20)->loadList($where,$limit);
            $count = target('dhcms/TotalSpiderInfo')->countList($where,$limit);
            $baidu = target('dhcms/TotalSpiderInfo')->getBaidu('http://www.baidu.com/s?wd=site:');
            $this->pager = target('dhcms/TotalSpiderInfo')->pager;
