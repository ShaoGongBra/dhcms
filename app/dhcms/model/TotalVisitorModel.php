...
class TotalVisitorModel extends BaseModel {
    ...
    public function curTotalNum(){
        $sql = "select sum(count) as s from {pre}total_visitor";
		$num = $this->query($sql);
		return $num[0]['s'];
    }
    ...
    public function curApi(){
        ...
        $where['time'] = $date;
        $info = $this->where($where)->find();
		$info['api'] = empty($info['api'])?0:$info['api'];
        return $info['api'];
    }
    ...
    public function curTotalApi(){
        $sql = "select sum(api) as s from {pre}total_visitor";
		$num = $this->query($sql);
		return $num[0]['s'];
    }
