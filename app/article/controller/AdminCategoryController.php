...
class AdminCategoryController extends AdminController {
    ...
    protected function _infoModule(){
        $menu = target('dhcms/AdminCategory','controller');
    ...
    public function add(){
        if(!IS_POST){
            $breadCrumb = array('栏目列表'=>url('dhcms/AdminCategory/index'),'文章栏目添加'=>url());
            $this->assign('breadCrumb',$breadCrumb);
            $this->assign('name','添加');
            $this->assign('categoryList',target('dhcms/Category')->loadList());
            $this->assign('tplList',target('admin/Config')->tplList());
            $this->assign('expandList',target('dhcms/FieldsetExpand')->loadList());
            $this->assign('default_config',current_config());
            $this->adminDisplay('info');
        }else{
            $_POST['app'] = APP_NAME;
            if(target('CategoryArticle')->saveData('add')){
                $this->success('栏目添加成功！',url('dhcms/AdminCategory/index'));
            }else{
    ...
    public function edit(){
        if(!IS_POST){
            ...
            $breadCrumb = array('栏目列表'=>url('dhcms/AdminCategory/index'),'文章栏目修改'=>url());
            $this->assign('breadCrumb',$breadCrumb);
            $this->assign('name','修改');
            $this->assign('categoryList',target('dhcms/Category')->loadList());
            $this->assign('tplList',target('admin/Config')->tplList());
            ...
            if(target('CategoryArticle')->saveData('edit')){
                $this->success('栏目修改成功！',url('dhcms/AdminCategory/index'));
            }else{
                $msg = target('CategoryArticle')->getError();
                if(empty($msg)){
                    $this->error('栏目修改失败');
    ...
    public function del(){
        ...
        //判断子栏目
        if(target('dhcms/Category')->loadList(array(), $classId)){
            $this->error('请先删除子栏目！');
        }
        //判断栏目下内容
        $where = array();
