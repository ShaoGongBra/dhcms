...
class CategoryController extends SiteController {
    ...
    public function index(){
        ...
        //获取条件参数
        $param = target('dhcms/Content')->stringToArray($select);
        //获取自定义查询条件
        $info['fieldset_id'] = $categoryInfo['fieldset_id'];
        $info['issearch'] = '1';
        $selects = target('dhcms/Field')->loadList($info);
        //自动生成多条件查询url 免去页面js传参
        foreach ($selects as $k => $v) {
            $condition[$v['field']]['name'] = $v['name'];
            $condition[$v['field']]['url'] = target('dhcms/Content')->route($v['field'],$param,'all',$url_info,$selects);
            ...
            if (!empty($v['config'])) {
                ...
                foreach ($fields as $a => $d) {
                    ...
                    $condition[$v['field']]['config'][$a]['url'] =  target('dhcms/Content')->route($v['field'],$param,$k,$url_info,$selects);
        ...
        //位置导航
        $crumb = target('dhcms/Category')->loadCrumb($classId);
        //设置查询条件
        $where='';
        if ($categoryInfo['type'] == 0) {
            $classIds = target('dhcms/Category')->getSubClassId($classId);
        }
        if(empty($classIds)){
        ...
        //获取顶级栏目信息
        $topCategoryInfo = target('dhcms/Category')->getInfo($crumb[0]['class_id']);
        //MEDIA信息
    ...
    public function filter(){
        ...
        foreach ($selects as $k => $v) {
            $condition[$v['field']]['name'] = $v['name'];
            $condition[$v['field']]['url'] = target('dhcms/Content')->route($v['field'],$param,'all',$url_info);
