...
class ContentController extends SiteController {
    ...
    public function index()
    {
        ...
        //位置导航
        $crumb = target('dhcms/Category')->loadCrumb($contentInfo['class_id']);
        //查询上级栏目信息
        ...
        target('dhcms/Content')->where($where)->setInc('views');
        //内容处理
        $contentInfo['content'] = html_out($contentInfo['content']);
        //扩展模型
        if($categoryInfo['fieldset_id']){
            $extInfo = target('dhcms/FieldsetExpand')->getDataInfo($categoryInfo['fieldset_id'],$contentId);
        ...
        if(!empty($prevInfo)){
            $prevInfo['aurl']=target('dhcms/Content')->getUrl($prevInfo,$appConfig);
            $prevInfo['curl']=target('dhcms/Category')->getUrl($prevInfo,$appConfig);
        }
        //下一篇
        $nextWhere = array();
        ...
        if(!empty($nextInfo)){
            $nextInfo['aurl']=target('dhcms/Content')->getUrl($nextInfo,$appConfig);
            $nextInfo['curl']=target('dhcms/Category')->getUrl($nextInfo,$appConfig);
        }
        //MEDIA信息
