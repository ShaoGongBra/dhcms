...
class ContentArticleModel extends BaseModel {
    ...
    public function loadList($where = array(), $limit = 0, $order = 'A.time desc,A.content_id desc', $fieldsetId = 0){
        ...
        //查询扩展信息
        if(!empty($fieldsetId)){
            $fieldsetInfo = target('dhcms/FieldsetExpand')->getInfo($fieldsetId);
            if(!empty($fieldsetInfo)){
                ...
                $fieldList = target('dhcms/FieldExpand')->loadList($whereExt);
            }
        }
        //获取最终结果
        ...
        if(!empty($pageList)){
            ...
            foreach ($pageList as $key=>$value) {
                ...
                $list[$key]['curl'] = target('dhcms/Category')->getUrl($value);
                $list[$key]['i'] = $i++;
                //处理扩展字段
                if(!empty($fieldList)){
                    foreach ($fieldList as $v) {
                        $list[$key][$v['field']] = target('dhcms/FieldData')->revertField($value[$v['field']],$v['type'],$v['config']);
                    }
                }
            }
        }
     ...
     * @return bool 更新状态
     */
    public function pingSucess($contentId)
    {
        $where['content_id'] = $contentId;
        $data['isping'] = 1;
        $status = $this->table('content')->where($where)->save($data);
        if($status === false){
            return false;
        }else{
            return true;
    ...
    public function saveData($type = 'add'){
        ...
        $contentId = target('dhcms/Content')->saveData($type);
        if(!$contentId){
            $this->error = target('dhcms/Content')->getError();
            return false;
        }
        //分表处理
        $data = $this->create();
